##### This is a file wine grading prediction models (PCA-LDA, kNN, SVM, XGBoost, ANN) and their local and global interpretations (LIME and SHAP).#####
...
### Part 8: LIME explainer ###
## ======================================================================= ##
## Initialise the explainer
dl_lime_explainer = lime.lime_tabular.LimeTabularExplainer(X_train.values, feature_names=X.columns, class_names=y_train, verbose=True, mode='classification')
# ---------------------------------------------------------------------------
## Explain a particular measurement in the test set
dl_lime = dl_lime_explainer.explain_instance(X_test.values[36], dl_model.predict, labels=wine_grades, num_features=10)
# ---------------------------------------------------------------------------
## Plot the LIME plots
def show_lime_plots():
    for i in [0,1,2]:
        print(dl_lime.as_pyplot_figure(label=i))
    print('All three LIME plots exported.')
show_lime_plots()
